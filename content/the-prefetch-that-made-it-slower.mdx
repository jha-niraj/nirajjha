...
Friday, late, and the last ticket in the column was an accessibility one. You could not tab to the rows in the orders table. They were `div`s with an `onClick`, which is the kind of thing that survives a year of code review because it looks fine and works with a mouse.

Thirty seconds of work. Swap them for links, push, go home.

Monday morning, support forwarded a complaint. The orders list had got slow. Not broken, just heavy, the way a page feels when it is quietly doing something it should not be.

The same three lines had made navigation faster and the list slower. Then I reached for the obvious fix and made it four times worse.
